...
function getDateFromTimeAgo(timeString) {
  ...
  const hoursAndMinutesAgoMatches = timeString.match(
    /^(\d+) (jam? dan) (\d+) (menit? yang lalu)/
  );
    ...
    isi.each((i, e) => {
      ...
      if (title != "") {
        const image_thumbnail = $(e)
          ...
          .children("img")
          .attr("src");
        const image_full = image_thumbnail
          ? image_thumbnail.replace("thumbnails2", "images")
          : "";
        ...
        let newTime = moment(time, "YYYY-MM-DD hh:mm:ss").format(
          "YYYY-MM-DD hh:mm"
        );
        result.push({
          title: title,
...
async function getDetail(slug) {
  ...
  try {
    ...
    let newTime = moment(theTime, "dddd, DD MMMM YYYY hh:mm").format(
      "YYYY-MM-DD hh:mm"
    );
    let medias = [];
...
async function getDataByKey(keyword, category = "news") {
  const url = base_url + "/" + category.toLowerCase();
  let result = [];
  try {
    const response = await axios.get(url);
